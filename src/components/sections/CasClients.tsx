      ...
      <Container>
        ...
        <ul className="mt-10 grid gap-5 lg:grid-cols-3">
          {casClients.map((cas) => (
            <li
              key={cas.id}
              className="grid grid-cols-[minmax(0,34%)_minmax(0,66%)] overflow-hidden rounded-xl border border-line bg-white"
            >
              <MediaSlot
                alt={cas.photo}
                src={cas.src}
              ...
              <div className="flex flex-col gap-2.5 p-5">
                <h3 className="text-base leading-snug font-semibold text-ink">{cas.titre}</h3>
                <p className="text-sm leading-relaxed text-muted">{cas.description}</p>
                ...
                <dl className="mt-auto flex flex-wrap gap-x-6 gap-y-3 pt-4">
                  {cas.chiffres.map((chiffre) => (
                    <div key={chiffre.libelle} className="flex flex-col">
                      <dd className="text-xl leading-none font-bold text-brand">{chiffre.valeur}</dd>
                      <dt className="mt-1 text-xs text-muted-light">{chiffre.libelle}</dt>
                    </div>
                  ))}
                </dl>
              </div>
